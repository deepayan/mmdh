             ...
             api_key = Sys.getenv("MOSPI_API_KEY"))
{
    ...
    if (length(files) == 0) {
        message(paste0("No files found for dataset '", id, "'."))
        return(character(0))
    }
    ...
    for (file_info in files) {
        ...
        url <- paste0(BASE_URL, "/fileslist/download/", id, "/", file_b64)
        ...
        tryCatch({
            ...
            message(paste0("Error downloading '", file_name, "': ", e$message))
        })
    }
